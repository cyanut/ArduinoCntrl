...
def gfxTimeDisplay(name, dataSource, timeSegment):
    ...
    for i in dataSource:
        ...
        if name == "tones":
            print bar + " [" + printDigits(i[1] / 1000, 3) + "s - " + printDigits(i[2] / 1000,
                                                                                  3) + "s][10][" + printDigits(i[3],
                                                                                                               4) + " Hz]"
        elif name == "simple outputs":
            print bar + " [" + printDigits(i[1] / 1000, 3) + "s - " + printDigits(i[2] / 1000, 3) + "s][" + printDigits(
                i[0], 2) + "][  n/a  ]"
        elif name == "frequency modulated outputs":
            print bar + " [" + printDigits(i[2] / 1000, 3) + "s - " + printDigits(i[3] / 1000, 3) + "s][" + printDigits(
                checkBin(i[5], "B")[0], 2) + "][" + printDigits(i[4], 4) + " Hz-" + printDigits(i[7],
                                                                                                2) + "%-" + printDigits(
                i[6], 3) + " deg]"
...
def get_data_errorCheck(ask_data, types, bestLen, starts, middles, ends, dataHold):
    ...
    try:
        ...
        if types == "pwm":
            ...
            refr = long(
                printDigits(freq, 5, True, "0") + printDigits(dutyCycle, 5, True, "0") + printDigits(phase, 5, True,
                                                                                                     "0"))
        ...
        if ask_data == "done":
            ...
            while analysisLoop == 0:
                ...
                if types == "pwm":
                    pinInt = posToInt(pins)
                    startsLrg, middlesLrg, endsLrg, dataHoldLrg = starts, middles, ends, dataHold
                    starts, middles, ends, dataHold = startsLrg[pins], middlesLrg[pins], endsLrg[pins], dataHoldLrg[
                        pinInt]
        ...
        for pinInt in dataHold:
            for refr in dataHold[pinInt]:
                ...
                for i in range(len(dataHold[pinInt][refr])):
                    if i % 2 == 0:
                        dataHoldDone.append(
                            [0, dataHold[pinInt][refr][i], dataHold[pinInt][refr][i + 1], freqi, pinInt, phasei, dutyi])
    return dataHoldDone
...
def run_setup(settings):
    ...
    if settings == "enter_new":
        ...
        while setupMainLoop == 0:
            ...
            print "(Note that the following graphical representation has an error within +/- [1 segment], or [" + str(
                timeSegment / 1000) + "s]."
    ...
    while True:
        ...
        while True:
            ...
            if barCounter == 0:
                ...
                barCounter, timeCounter = min(barCounter, 99), min(timeCounter, float(packet[3] / 1000) - 0.1)
                sys.stdout.write(
                    "\r" + "-" * (barCounter - 1) + "|" + "-" * (100 - barCounter) + " [" + str(timeCounter) + "s]")
                sys.stdout.flush()
    ...
    def streamConfig(self, NumChannels=1, ResolutionIndex=0, SamplesPerPacket=25, SettlingFactor=0,
        ...
        if (ScanFrequency is not None) or (SampleFrequency is not None):
            ...
            if ScanFrequency < 1000:
                if ScanFrequency < 25:
                    SamplesPerPacket = findSamplesPerPacket(ScanFrequency,
                                                            NumChannels)  # below 25 ScanFreq, S/P is some multiple of nCh less than SF.
...
def setupLabJack():
    ...
    ljSettings = "Sampling %s Channels:\n[Channel Numbers ]: %s\n[Scan Frequency  ]: %sHz/Channel  |  %sHz Total" % (
    nCh, chNum, scanFreq, nCh * scanFreq)
    ...
    while LJLoop1 == 0:
        ...
        if askLJ == "y":
            ...
            for i in allFiles:
                f = open(prgmDir + i, "r")
                print "[" + i[:-4].upper() + "]: Channels: " + f.readline()[:-1] + "; Scan Frequency: " + f.readline()[
                                                                                                          :-1] + "\n"
                f.close()
            print "#" * 10 + "\n"
            while True:
...
class StreamDataReader(object):
    ...
    def readStreamData(self):
        ...
        beforeStart = float((timeStart - timeStartRead).seconds * 1000) + float(
            (timeStart - timeStartRead).microseconds) / 1000
        afterStop = float((stopRead - stop).seconds * 1000) + float((stopRead - stop).microseconds) / 1000
        ttlRunTime = float((stopRead - timeStartRead).seconds * 1000) + float(
            (stopRead - timeStartRead).microseconds) / 1000
        ...
        if len(sdrMissedList) != 0:
            for i in sdrMissedList:
                if i[1] <= float(int(beforeStart)) / 1000:
                    missedBefore += i[0]
                elif i[1] > float(int(beforeStart)) / 1000 and i[1] <= float(int(beforeStart)) / 1000 + float(
                        int(runTime)) / 1000:
                    missedDuring += i[0]
                elif i[1] > float(int(beforeStart)) / 1000 + float(int(runTime)) / 1000 and i[1] <= float(
                        int(beforeStart)) / 1000 + float(int(runTime)) / 1000 + float(int(afterStop)) / 1000:
                    missedAfter += i[0]
        while True:
            warning = False
            if sdrMissed != 0:
            ...
            if runDone == 1:
                ...
                print "     SAMPLES TAKEN|%s|     %s     |%s|%s|" % (
                printDigits(sdrTotalAux1, 8), printDigits(sdrTotalMain, 9), printDigits(sdrTotalAux2, 8),
                printDigits(sdrTotal, 8))
                print "    SAMPLES MISSED|%s|     %s     |%s|%s|" % (
                printDigits(missedBefore, 8), printDigits(missedDuring, 9), printDigits(missedAfter, 8),
                printDigits(sdrMissed, 8))
                print "SAMPLING FREQ (Hz)|        |     %s     |        |%s|" % (
                printDigits(finalSmplFreqExp, 9), printDigits(finalSmplFreq, 8))
                print "    SCAN FREQ (Hz)|        |     %s     |        |%s|" % (
        ...
        while True:
            try:
                ...
                if result['errors'] != 0:
                    ...
                    timeDiff = str(
                        (missedTime - timeStartRead).seconds * 1000 + (missedTime - timeStartRead).microseconds / 1000)
            ...
            if runDone == 2:
                if hiddenOption == "p":
                    ...
                    timeLine = "TIME (s),%s,%s,%s,%s,\n" % (printDigits(float(int(beforeStart)) / 1000, 8),
                                                            printDigits(float(int(runTime)) / 1000, 9),
                                                            printDigits(float(int(afterStop)) / 1000, 8),
                ...
                with file(resultsDir + ljSaveName + ".csv", "r") as original:
                    data = original.read()
                with file(resultsDir + ljSaveName + ".csv", "w") as modified:
                    modified.write(stats + data)
                break
            time.sleep(0.00001)
...
if hiddenOption == "p":
    ...
    with open(prgmDir + "settings.txt", "r") as f:
        ...
        if photometryConfig1.startswith("N/A"):
            photometryConfig1 = raw_input(
                "[LABJACK CHANNEL FORMAT]: True Data, True Reference, Isosbestic Data, Isosbestic Reference").strip().split(
                ",")
            photometryConfig2 = raw_input(
                "[ REFERENCE FREQ FORMAT]: True Reference Freq Hz, Isosbestic Reference Freq Hz").strip().split(",")
        else:
            photometryConfig1, photometryConfig2 = ast.literal_eval(photometryConfig1), ast.literal_eval(
                photometryConfig2)
            print "EXISTING PHOTOMETRY OPTIONS: "
            print "True Data Channel: {}; True Ref Channel: {}".format(photometryConfig1[0], photometryConfig1[1])
            print "Isosbestic Data Channel: {}; Isosbestic Ref Channel: {}".format(photometryConfig1[2],
                                                                                   photometryConfig1[3])
